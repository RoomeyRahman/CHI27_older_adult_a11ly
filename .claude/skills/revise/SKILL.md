...
4. **Fact-Checking:** If the reviewer asks for expanded data or justifications, immediately cross-reference, in the precedence order of CLAUDE.md Section 3: `/output/Method.md` for anything about what was done, counts, or the deployed feature list (the ceiling for any autonomy claim; `/system/` does not exist and no logs exist); `/output/codes/A2/` for any finding or extract (A1 is audit trail only); `/Supplementary/Interviews/phase-1/` and `phase-2/` to verify an extract at its cited line; `/output/Related-Work.md` for positioning; and `/proposal/proposal.md` for framing, RQs, and contributions only. There is no `current_plan.md`, `rqs.md`, `motivation.md`, or `research_directions.md` in this repository, and CLAUDE.md's own summary of the data is a checksum, not a source.
...
- **Tension Impact:** Does the requested change sand down a seeded tension (CLAUDE.md Section 9.3: delegated dependence as agency, reminders strengthen felt timing without replacing memory, oversight as intimacy, streak grief, trust through self-verification, each anchored to a named A2 theme)? If so, propose a revision that answers the reviewer while preserving the tension and its counter-case.
