...
8. **Participant names.** A real participant name, employer, or workplace may not enter the LaTeX. Older adults and caregivers appear by ID only; caregiver transcripts are not de-identified (CLAUDE.md Section 3.4), so if the markdown carries a name, stop and report it rather than transcribing it.
...
The deployed prototype is described inside `/output/Method.md` (Prototype Development, with Figures 1 and 2), so there is no separate System slot; a `system` or `design` argument is treated as a section with no slot under the last rule below, and the user is asked before a new file is created. `sources/2_background.tex` and `sources/3_method.tex` are the LaTeX twins of `/output/Related-Work.md` and `/output/Method.md` (CLAUDE.md Section 4); a change to either markdown re-runs this skill on that section.
...
```bash
python3 .claude/skills/latex/scripts/md2tex.py output/Method.md \
