...
(CLAUDE.md Section 3.4). **No real name, employer, or workplace ever enters `/output/codes/`, in any slot.** This
...
(CLAUDE.md Section 3.5). A pairing is asserted only where a transcript itself states the relation, and then only as
...
**Names and CLAUDE.md.** CLAUDE.md lists no participant names and never will. It records only that the caregiver
...
Consent in a collectivist household is familial as well as individual (CLAUDE.md Section 9.4); `/output/Method.md`
