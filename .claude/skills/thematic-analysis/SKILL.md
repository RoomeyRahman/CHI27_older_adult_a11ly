...
```bash
bash .claude/skills/thematic-analysis/scripts/slot.sh
```
...
- The scripts take the slot as a second argument, so they check only your own work:
  ...
  ```bash
  python3 .claude/skills/thematic-analysis/scripts/quote_check.py . A1
  python3 .claude/skills/thematic-analysis/scripts/table_check.py . A1
  bash .claude/skills/thematic-analysis/scripts/anon_scan.sh . A1
  ```
...
```
/output/codes/
  A1/                    Claude Code's analysis          } one slot per agent, never mixed,
...
`README.md` names the agent that produced the slot in its first line, so a reader opening `/output/codes/A2/` knows
...
- **Framing commitments (CLAUDE.md 2.4)** hold everywhere. A design gap rather than a memory problem, so no code
...
- **Terminology (CLAUDE.md 2.5 and 7.8).** The anchor terms *care network*, *allegiance*, *the agent*, *dignity*,
...
```bash
python3 .claude/skills/thematic-analysis/scripts/quote_check.py . <slot>   # every extract verbatim in its transcript
