...
| `/.claude/skills/`           | `plan-section/`, `draft/`, `revise/`, `polish/`, `grill/`, `thematic-analysis/`, `latex/`, `chi-evidence-matrix/`, `chi-introduction/`, `chi-literature-scout/`, `chi-litreview-writer/`, `chi-research-question/` | **Editing source of truth for skill text.** Section planning, drafting, revision, style calibration, adversarial review, analysis, ACM conversion, literature work, RQ stewardship |
| `/.codex/skills/`            | The same twelve skills in Codex form, generated by `.codex/sync-skills.sh`                 | Never hand edit. After any change under `/.claude/skills/`, run `bash .codex/sync-skills.sh` in the same task |
