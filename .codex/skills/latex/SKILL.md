---
name: latex
description: Converts a finished markdown section in /output/ into ACM (acmart) LaTeX inside /output/latex/CHI27_older_adult_accessibility/, converting numeric markdown citations into \cite{} against reference.bib, writing the section to its sources/*.tex file, and updating main.tex accordingly. Use when a drafted section needs to enter the compiled submission.
argument-hint: [section-name | path-to-md | abstract | all]
---

We are moving `$1` from markdown into the compiled ACM submission. This is a **transcription task, not a writing task**: the prose is already final, and this pass changes markup only. The target project is `/output/latex/CHI27_older_adult_accessibility/`, built with `acmart` (`\documentclass[manuscript,review,anonymous]{acmart}`) and compiled from the repository root with `make pdf`.

Two helper scripts live beside this file and do the mechanical work:

- `.codex/skills/latex/scripts/md2tex.py` converts one markdown file to LaTeX and emits a JSON report of every citation, placeholder, dash, and table it found.
- `.codex/skills/latex/scripts/check_cites.py` verifies every `\cite` key in a `.tex` file resolves against the bib file(s).
...
7. **Corpus and analysis are read-only.** Never edit anything under `/Supplementary/` or `/output/codes/`.
8. **Participant names.** A real participant name, employer, or workplace may not enter the LaTeX. Older adults and caregivers appear by ID only; caregiver transcripts are not de-identified (AGENTS.md Section 3.4), so if the markdown carries a name, stop and report it rather than transcribing it.

### Phase 0.5: Project Bootstrap Check

Before resolving the target, confirm `/output/latex/CHI27_older_adult_accessibility/` exists and holds `main.tex`, `acmart.cls`, `ACM-Reference-Format.bst`, and a `sources/` directory.
...
| `background`, `related`, `related work` | `/output/Related-Work.md` | `sources/2_background.tex` | Related Work |
| `method`, `methods`, `methodology` | `/output/Method.md` | `sources/3_method.tex` | Method |
| `findings`, `results` | `/output/Findings.md` | `sources/4_findings.tex` | Findings |
| `discussion` | `/output/Discussion.md` | `sources/5_discussion.tex` | Discussion |
| `conclusion`, `limitations` | `/output/Conclusion.md` or `/output/Limitations.md` | `sources/6_conclusion.tex` | (see note) |
...
The deployed prototype is described inside `/output/Method.md` (Prototype Development, with Figures 1 and 2), so there is no separate System slot; a `system` or `design` argument is treated as a section with no slot under the last rule below, and the user is asked before a new file is created. `sources/2_background.tex` and `sources/3_method.tex` are the LaTeX twins of `/output/Related-Work.md` and `/output/Method.md` (AGENTS.md Section 4); a change to either markdown re-runs this skill on that section.
...
```bash
python3 .codex/skills/latex/scripts/md2tex.py output/Method.md \
...
1. **Participant quotations.** A quotation the markdown sets as a block quote belongs in `itquote`. A short in-line quotation stays in-line with `` `` '' `` quotes. Attribution stays exactly as the markdown wrote it, including the older adult or caregiver role label where the markdown carries one. Verify every quotation is character-identical to the markdown, which is itself verbatim from an A2 extract in `/output/codes/A2/` or a filed transcript line in `/Supplementary/Interviews/`. A quotation translated from Bangla keeps its translation exactly as the markdown gives it; never re-translate, and never typeset the Bangla original in place of the filed translation.
...
4. **Tables.** The script comments a markdown table out and leaves a `TODO(/latex)` line. Build a real `table`/`tabular` with `\caption`, `\label{tab:...}`, and `booktabs` rules (already loaded), place it near its first reference, and add a `\ref{}` if the prose refers to it. Report every table you built. A participant table (Method Tables 1 and 2) keeps its participant IDs and every cell exactly as filed, including "Not recorded".
5. **Figures.** Only insert a figure when the markdown references one, and only when the image file already exists in the project's `Figures/` directory (currently `initial uses.jpg`, the design-concept storyboard, and `relational-trigger-loop.png`, the deployed interaction loop); caption text comes from the markdown, never from you. Any other diagram is a figure the markdown must call for first, and no figure may depict a mechanism that was not deployed as though it were (AGENTS.md Section 3.3).
...
7. **Dashes.** The report lists every `--` or `---` the source contained. AGENTS.md 7.2 forbids dashes in this paper's prose, so do not convert them into typeset dashes: flag each one for the user and leave the source markdown for `$polish` to fix.
...
The markdown cites by number and `/references/reference.bib` uses those same numbers as its entry keys, so `[27, 34]` resolves to `\cite{27,34}` directly. Where the drafting skills emitted verified filed sources as `[Author Year; filename.pdf]` rather than numbers, those are not yet bib keys: report each one as needing a numbered entry in `/references/reference.bib` before it can compile, and leave it as a `\suggestion{}` marker rather than inventing a key.

1. Copy `/references/reference.bib` into the project as `reference.bib` whenever the project copy is missing or differs from it. Leave `sample-base.bib` on disk untouched.
...
```bash
python3 .codex/skills/latex/scripts/check_cites.py \
  output/latex/CHI27_older_adult_accessibility/sources/3_method.tex \
  output/latex/CHI27_older_adult_accessibility/reference.bib
```
...
2. **Introduction (`intro` argument), additional steps.**
   - If `\title{}` is empty, fill it with the working title from AGENTS.md 2.1, `Who Does the AI Work For? Negotiating an AI Agent's Role Between Older Adults and Family Caregivers in Bangladesh`, and note in the report that it is the working title, not a final one. If `\title{}` is already populated, leave it.
...
The Makefile's `PROJECT` variable defaults to `CHI27_older_adult_accessibility`; override it with `make pdf PROJECT=other_folder` only when building a different project under `output/latex/`. If Docker is unavailable, say so plainly and report the conversion as unverified rather than claiming a build. On a build:

1. Read `output/latex/CHI27_older_adult_accessibility/main.log` for errors and for `Undefined control sequence`, `Missing $ inserted`, `Runaway argument`, and `LaTeX Warning: Citation ... undefined`.
...
5. **Dashes found in the source:** each one with its line, flagged for `$polish`, since AGENTS.md 7.2 bans them.
