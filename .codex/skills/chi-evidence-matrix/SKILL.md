...
**Standing use in this repository.** AGENTS.md Section 2.6 names three literatures the paper must occupy the
...
```bash
python3 .codex/skills/chi-evidence-matrix/scripts/bib_index.py <bib> \
...
Write the result to `$SCRATCH/work/synthesis.json` (schema: `python3 .codex/skills/chi-evidence-matrix/scripts/build_matrix.py --schema`).

### Stage 6 — Build and deliver

```bash
python3 .codex/skills/chi-evidence-matrix/scripts/build_matrix.py "$SCRATCH/work/" \
...
- **Answering the last column.** Leaving it open is the point of this pass. In this repository it is also a rule:
  AGENTS.md Section 6 forbids drafting our own findings into a literature artifact, and the last column stays
...
- **A priority claim smuggled into a knowledge statement.** "No prior work has studied X" is unwriteable here
  (AGENTS.md Section 2.5). State the gap descriptively so it survives a near-neighbour surfacing.
