...
1. **`/proposal/proposal.md` in full.** This is the single canonical framing document (AGENTS.md Sections 2, 3.4): main idea, novelty, motivation, the three RQs of Section 4, and the compound contribution C1 to C3 of Section 5. Nothing overrides it on framing. There is no `current_plan.md`, no `rqs.md`, no `motivation.md`, and no `research_directions.md` in this repository; if a document with a deficit framing of older adults surfaces anywhere, it is superseded and is not a source (AGENTS.md Section 2).
2. **`/supplementary/` in full**, for facts and quotable material relevant to `$1`: `/supplementary/formative/` (Study 1, the 26 formative interviews and their session records), `/supplementary/deployment/` (Study 2, the six evaluation interviews, build notes, and the feature specification), and `/supplementary/household/` (Study 3, empty until that study runs). These files are the sole source for every empirical claim; AGENTS.md's own summary of the data is not a citable source.
...
6. `/Training/writing-style.md` in full (the canonical style guide, AGENTS.md Section 7): the plan must set `$draft` up to satisfy it, in particular by planning where the section's contrast frames land (the "rather than X, Y" positioning moves, six to eight per thousand words spread across the section, one per related-work gap, each naming an alternative its advocates would recognise) and by respecting the section-tuning profile (Introduction and Discussion argue with dense frames; Method plans flat exact declaratives; Findings plan families and participants in subject position with quotes carrying evidence; Limitations plan the flattest prose in the paper). The paper's six standing contrast frames are listed in AGENTS.md Section 7.4; a section's frames should mostly be drawn from them. Then the `/Training/` PDFs for CHI structural standards relevant to `$1`.

Cross-check every count you plan to reference against AGENTS.md Section 3, and every count in AGENTS.md Section 3 against the filed source before the plan commits to it. Three supersessions bind every plan (AGENTS.md Sections 2, 2.3, 3.2):
...
- **For any other section:** State its purpose in one sentence, then decompose into beats, each mapped to an RQ per AGENTS.md Section 2.3 ("Every section draft must state, at least implicitly, which RQ it advances").

### Phase 3: Theory Alignment for the Plan

Produce the Theory Alignment block required by AGENTS.md Section 10, scoped to what this section's plan proposes:

- **Primary framework(s) proposed for this section**, with key constructs, drawn from the candidate stack in AGENTS.md Section 10 or newly identified in Phase 4.
...
**Standing coverage debt.** `/references/` currently leans toward caregiving and aging reviews. The alignment and principal-agent literature, the CSCW caregiver-dashboard literature, and the postcolonial-computing and intermediated-use literature are the three the positioning of AGENTS.md Section 2.6 depends on, and they are the thinnest on disk. Weight the sweep toward whichever of the three `$1` actually needs, and report the per-literature count in the Phase 8 summary.
...
- The RQ(s) it advances (AGENTS.md Section 2.3).
...
- Whether it protects a seeded tension (AGENTS.md Section 9.3: delegated dependence as agency, memory restored rather than replaced, oversight as intimacy, streak grief, trust through self-verification) that must not be sanded smooth. Every tension beat also names the counter-case the tension requires before it may anchor a claim.
...
- Whether it carries one of the section's planned contrast frames (`/Training/writing-style.md`, and AGENTS.md Section 7.4 for the six standing frames), and if so, the alternative it names; the thesis, each defined construct, each related-work gap, and the close each get one.

Do not let any beat rest on a claim with no traceable source; convert it to `[MISSING DATA]` instead of writing around the gap. A beat whose only support is AGENTS.md's own summary of the data is `[MISSING DATA]` until the filed source is located.
...
Before writing the plan file, verify it would let a subsequent `$draft` pass the Section Quality Gates (AGENTS.md Section 11):
...
3. Evidence gate: every beat has a filed source, a `[cite]`, a Phase 4 candidate, `[MISSING DATA]`, or `[BLOCKED]`. Counts match AGENTS.md Section 3 exactly. No allegiance or network beat is sourced to Study 2.
