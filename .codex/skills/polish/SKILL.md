...
2. **Quotes verbatim.** Participant quotations are untouchable: no rewording, no trimming without ellipsis already present in the source, no new quotes, and no gloss added after one. No unconfirmed real participant name may be introduced or left standing (AGENTS.md Section 3.2 pseudonym rule).
...
- Somewhere, plainly, what the work does not cover; limitations in the flattest prose in the paper, each framed as a scoping decision (AGENTS.md 6.2 and 6.3): one cultural setting, Study 2's young and individual sample, deployment lengths, self-report plus logs rather than health outcomes, the agent's bounded autonomy. Never extend a claim from Bangladesh to the Global South at large.
